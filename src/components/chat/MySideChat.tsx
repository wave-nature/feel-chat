...
function MySideChat({ chat }: any) {
  const { playing, timer, imageUrl, setPlaying, setAudioUrl, setImageUrl } =
    useSong(chat);
    ...
    <div className="col-start-6 col-end-13 p-3 rounded-lg">
      <div className="flex items-center justify-start flex-row-reverse">
        ...
        <div className="relative mr-3 text-sm bg-indigo-100 py-2 px-4 shadow rounded-xl">
              ...
              <button className="flex items-center justify-center bg-indigo-600 hover:bg-indigo-800 rounded-full h-8 w-10">
                {playing ? (
                  ...
                  <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                    onClick={() => {
                      const audioUrl = chat?.feel_data?.data?.song?.streams[0];
                      setPlaying(true);
          ...
          {/* progress bar */}
          {feelSong && (
            <div className="w-full bg-gray-400 rounded-full h-2.5 mt-2 dark:bg-gray-700">
              <div
